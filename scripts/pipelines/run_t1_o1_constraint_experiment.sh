...
# 2) Train with the O1 projected-dual fairness constraint
...
RUN_NAME="bert_to_bert-tiny_all_patients_o1_gender_fair_teacher_o1"
...
echo "========================================================================"
echo "▶  T1 + O1: Distill from fair teacher with constrained fairness"
...
if [[ -n "$RUN_CHECKPOINT" ]]; then
  ...
  "$PYTHON_BIN" distillation/scripts/distill_students.py \
    ...
    --fairness-constraint \
    --fairness-constraint-epsilon 0.05 \
    --fairness-dual-lr 0.01 \
    --fairness-dual-init 1.0 \
    --dir-suffix _fair_teacher_o1

  RUN_CHECKPOINT=$(find "$RUN_DIR" -name "student_distilled.pth" 2>/dev/null | sort | tail -1 || true)
fi

if [[ -z "$RUN_CHECKPOINT" ]]; then
  echo "T1+O1 checkpoint not found under: $RUN_DIR"
  exit 1
fi
...
if [[ -f "$RUN_RESULTS" ]] && [[ $(wc -l < "$RUN_RESULTS") -ge 13 ]]; then
  ...
  run_per_patient_inference "$RUN_CHECKPOINT" "$RUN_INFERENCE_DIR" "T1+O1"
fi

echo "[T1+O1] Update fairness comparison with leakage-free patient holdout calibration"
"$PYTHON_BIN" scripts/fairness/compute_fairness_comparison.py \
  --pipeline-dir "$PIPELINE_DIR" \
  --calibration-mode patient-holdout \
  --calibration-folds 2

echo "[T1+O1] Done. Outputs:"
