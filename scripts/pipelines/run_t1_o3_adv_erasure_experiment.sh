...
# T1 + O3 experiment
# 1) Distill student from the fair teacher checkpoint with adversarial group erasure
#    (gradient-reversal discriminator predicts gender from student hidden states;
#     student is penalized for being gender-predictable → group-invariant features)
# 2) Run per-patient inference
# 3) Refresh the leakage-free fairness comparison table
#
# O3 attacks the gap by a different mechanism than every other method: it erases
# group information from the representation rather than reweighting the loss.
# Tuning: ADV_ERASURE_LAMBDA is the gradient-reversal strength (student de-bias
# weight). Too high → underfitting; watch RMSE. Watch the "O3 Adv" term in the
# log: it should be near ln(2)=0.69 if the discriminator is at chance (= erased),
# and lower if the student is still gender-predictable.
...
ADV_ERASURE_LAMBDA="${ADV_ERASURE_LAMBDA:-1.0}"
...
RUN_NAME="bert_to_bert-tiny_all_patients_o3adv_gender_fair_teacher"
...
echo "========================================================================"
echo "▶  T1 + O3: Distill from fair teacher with adversarial group erasure"
echo "   Pipeline:  $PIPELINE_DIR"
echo "   Run dir:   $RUN_DIR"
echo "   Lambda:    $ADV_ERASURE_LAMBDA"
echo "========================================================================"
...
if [[ -n "$RUN_CHECKPOINT" ]]; then
  ...
  "$PYTHON_BIN" distillation/scripts/distill_students.py \
    ...
    --adv-erasure \
    --adv-erasure-feature gender \
    --adv-erasure-lambda "$ADV_ERASURE_LAMBDA" \
    --adv-erasure-layer -1 \
...
if [[ -z "$RUN_CHECKPOINT" ]]; then
  echo "T1+O3 checkpoint not found under: $RUN_DIR"
  exit 1
fi
...
if [[ -f "$RUN_RESULTS" ]] && [[ $(wc -l < "$RUN_RESULTS") -ge 13 ]]; then
  ...
  run_per_patient_inference "$RUN_CHECKPOINT" "$RUN_INFERENCE_DIR" "T1+O3"
fi

echo "[T1+O3] Update fairness comparison with leakage-free patient holdout calibration"
"$PYTHON_BIN" scripts/fairness/compute_fairness_comparison.py \
  --pipeline-dir "$PIPELINE_DIR" \
  --calibration-mode patient-holdout \
  --calibration-folds 2

echo "[T1+O3] Done. Outputs:"
