...
# T1 + K1 experiment
# 1) Compute K1 offsets from the fair teacher's comparable inference outputs
# 2) Distill student from the fair teacher checkpoint with calibrated soft labels
# 3) Run per-patient inference and refresh the leakage-free fairness table
...
FAIR_TEACHER_INFERENCE="$PHASE1_DIR/per_patient_inference_fair_teacher/time_llm_per_patient_inference_ohiot1dm"
K1_OFFSET_JSON="$PHASE1_DIR/k1_teacher_offsets_gender_fair_teacher.json"
RUN_NAME="bert_to_bert-tiny_all_patients_k1cal_gender_fair_teacher"
...
if [[ ! -d "$FAIR_TEACHER_INFERENCE" ]]; then
  echo "Missing fair teacher inference directory: $FAIR_TEACHER_INFERENCE"
  exit 1
fi
...
echo "========================================================================"
echo "▶  T1 + K1: Distill from fair teacher with calibrated soft labels"
echo "   Pipeline: $PIPELINE_DIR"
echo "   Run dir:  $RUN_DIR"
echo "========================================================================"

echo "[T1+K1] Compute fair-teacher K1 offsets"
"$PYTHON_BIN" scripts/fairness/compute_k1_teacher_offsets.py \
  --inference-dir "$FAIR_TEACHER_INFERENCE" \
  --feature gender \
  --group0 Female \
  --group1 Male \
  --base-threshold 70.0 \
...
if [[ -n "$RUN_CHECKPOINT" ]]; then
  ...
  "$PYTHON_BIN" distillation/scripts/distill_students.py \
    ...
    --teacher-calibration \
    --teacher-calibration-feature gender \
...
if [[ -z "$RUN_CHECKPOINT" ]]; then
  echo "T1+K1 checkpoint not found under: $RUN_DIR"
  exit 1
fi
...
if [[ -f "$RUN_RESULTS" ]] && [[ $(wc -l < "$RUN_RESULTS") -ge 13 ]]; then
  ...
  run_per_patient_inference "$RUN_CHECKPOINT" "$RUN_INFERENCE_DIR" "T1+K1"
fi

echo "[T1+K1] Update fairness comparison with leakage-free patient holdout calibration"
"$PYTHON_BIN" scripts/fairness/compute_fairness_comparison.py \
  --pipeline-dir "$PIPELINE_DIR" \
  --calibration-mode patient-holdout \
  --calibration-folds 2

echo "[T1+K1] Done. Outputs:"
