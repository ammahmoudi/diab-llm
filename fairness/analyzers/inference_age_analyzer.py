#!/usr/bin/env python3
"""
Age Group Fairness Analyzer for Inference Scenarios

Analyzes fairness across age groups (20-40, 40-60, 60-80) for:
...
class InferenceAgeAnalyzer(BaseInferenceAnalyzer):
    """Analyze age group fairness across inference scenarios."""
    
    def __init__(self, data_path=None):
        super().__init__(feature_name="Age Group", data_path=data_path or "")
    
    def get_feature_value(self, patient_id: str) -> str:
        """Get age group for a specific patient."""
        return self.patient_data.get(patient_id, {}).get('age', 'Unknown')
    
    def analyze(self):
        """Run comprehensive age group fairness analysis across all scenarios."""
        print("\n" + "="*70)
        print("AGE GROUP FAIRNESS ANALYSIS ACROSS INFERENCE SCENARIOS")
        print("="*70)
    ...
    def _generate_report(self, all_statistics, all_fairness, comparison, scenario_impacts):
        ...
        report_file = self.results_dir / f"age_inference_scenarios_report_{timestamp}.json"
    ...
    def _create_visualization(self, all_statistics, all_fairness, comparison):
        ...
        fig.suptitle('Age Group Fairness Analysis - Inference Scenarios Comparison', 
        ...
        age_groups = sorted(list(all_groups))
        
        table_data = [['Scenario'] + [f'{g}\nRMSE' for g in age_groups] + ['Fairness\nRatio', 'Level']]
        ...
        plot_file = self.results_dir / f"age_inference_scenarios_{timestamp}.png"
    ...
    def _plot_multi_scenario_comparison(self, ax, all_statistics, metric_key, title, ylabel):
        ...
        colors = {'20-40': '#3498db', '40-60': '#2ecc71', '60-80': '#e74c3c'}
        ...
        for i, group in enumerate(age_groups):
            ...
            offset = width * (i - len(age_groups)/2 + 0.5)
