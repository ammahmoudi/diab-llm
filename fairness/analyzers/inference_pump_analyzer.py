#!/usr/bin/env python3
"""
Pump Model Fairness Analyzer for Inference Scenarios

Analyzes fairness across pump models (630G, 530G) for:
...
class InferencePumpAnalyzer(BaseInferenceAnalyzer):
    """Analyze pump model fairness across inference scenarios."""
    
    def __init__(self, data_path=None):
        super().__init__(feature_name="Pump Model", data_path=data_path or "")
    
    def get_feature_value(self, patient_id: str) -> str:
        """Get pump model for a specific patient."""
        return self.patient_data.get(patient_id, {}).get('pump_model', 'Unknown')
    
    def analyze(self):
        """Run comprehensive pump model fairness analysis across all scenarios."""
        print("\n" + "="*70)
        print("PUMP MODEL FAIRNESS ANALYSIS ACROSS INFERENCE SCENARIOS")
        print("="*70)
        ...
        print(f"\n📊 PERFORMANCE BY PUMP MODEL ACROSS SCENARIOS:")
    ...
    def _generate_report(self, all_statistics, all_fairness, comparison, scenario_impacts):
        ...
        report = {
            "report_type": "Age Group Fairness Analysis - Inference Scenarios",
        ...
        report_file = self.results_dir / f"pump_inference_scenarios_report_{timestamp}.json"
    ...
    def _create_visualization(self, all_statistics, all_fairness, comparison):
        ...
        plot_file = self.results_dir / f"pump_inference_scenarios_{timestamp}.png"
    ...
    def _plot_multi_scenario_comparison(self, ax, all_statistics, metric_key, title, ylabel):
        ...
        colors = {'630G': '#3498db', '530G': '#e74c3c'}
        
        for i, group in enumerate(pump_models):
            values = []
            ...
            offset = width * (i - len(pump_models)/2 + 0.5)
...
def main():
    ...
    analyzer = InferencePumpAnalyzer()
    analyzer.analyze()
